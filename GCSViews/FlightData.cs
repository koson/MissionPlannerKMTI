...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        private void FlightData_ParentChanged(object sender, EventArgs e)
        {
            ...
            if (MainV2.comPort.MAV.param.ContainsKey("PARA_FS_HEIGHT"))
            {
                Fs_alt.Value = (decimal)(float)MainV2.comPort.MAV.param["PARA_FS_HEIGHT"];
            }
            if (MainV2.comPort.MAV.param.ContainsKey("ALT_HOLD_RTL"))
            {
                modifyandSetRTL.Value = (decimal)((float)MainV2.comPort.MAV.param["ALT_HOLD_RTL"] / 100.0);
            }
            if (MainV2.comPort.MAV.param.ContainsKey("WP_LOITER_RAD"))
            {
                modifyandSetLoiter.Value = (decimal)(float)MainV2.comPort.MAV.param["WP_LOITER_RAD"];
            }
...
";
        ...
        private void fs_on_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 11, 2000, 0, 0, 0, 0, 0))
                {
                ...
                else
                {
                    ...
                    catch { CustomMessageBox.Show("Error setting GSC FS ", "Error"); }
                }
            }
        ...
        private void fs_off_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 11, 1000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
        ...
        private void st_fs_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 10, 2000, 0, 0, 0, 0, 0))
                {
                    ...
                    emg_state.BackColor = Color.Red;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void rs_fs_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 10, 1000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    emg_state.BackColor = Color.Green;

            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }
        private void Fs_alt_Click(object sender, EventArgs e)
        {
            float newalt = (float)Fs_alt.Value;
            try
            {
                MainV2.comPort.setParam("PARA_FS_HEIGHT", newalt);
            }
            catch { CustomMessageBox.Show("Error sending new Alt", "Error"); }
            //MainV2.comPort.setNextWPTargetAlt((ushort)MainV2.comPort.MAV.cs.wpno, newalt);
        }
        private void modifyandSetLoiter_Click(object sender, EventArgs e)
        {
            float newrad = (float)modifyandSetLoiter.Value;
            try
            {
                MainV2.comPort.setParam("WP_LOITER_RAD", newrad);
            }
            catch { CustomMessageBox.Show("Error sending new Loiter rad", "Error"); }
            //MainV2.comPort.setNextWPTargetAlt((ushort)MainV2.comPort.MAV.cs.wpno, newalt);
        }
        private void modifyandSetRTL_Click(object sender, EventArgs e)
        {
            float newRTL = (float)((float)modifyandSetRTL.Value * 100.0);
            try
            {
                MainV2.comPort.setParam("ALT_HOLD_RTL", newRTL);
            }
            catch { CustomMessageBox.Show("Error sending new RTL alt", "Error"); }
            //MainV2.comPort.setNextWPTargetAlt((ushort)MainV2.comPort.MAV.cs.wpno, newalt);
        }

        private void nav_leds_on_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 7, 2000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    nav_leds.BackColor = Color.Green;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void nav_leds_off_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 7, 1000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    nav_leds.BackColor = Color.Red;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void left_flare_on_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 5, 2000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    left_flare.BackColor = Color.Green;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void left_flare_off_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 5, 1000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    left_flare.BackColor = Color.Red;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void right_flare_on_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 6, 2000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    right_flare.BackColor = Color.Green;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        private void right_flare_off_Click(object sender, EventArgs e)
        {
            try
            {
                if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 6, 1000, 0, 0, 0, 0, 0))
                {
                    CustomMessageBox.Show("Command Failed", "Error");
                }
                else
                    right_flare.BackColor = Color.Red;
            }
            catch (Exception ex) { CustomMessageBox.Show("Command Failed " + ex.ToString(), "Error"); }
        }

        /*private void Fs_alt_Load(object sender, EventArgs e)
        {
            Fs_alt.Value = (decimal)MainV2.comPort.MAV.param["PARA_HEIGHT"];
        }*/
    }
}
